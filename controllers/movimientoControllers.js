...
const { refactorizarMovOuput, refactorOutput } = require("../utils/refactor");
const { calcularSaldo, calcTotal } = require("../utils/calculos");
...
exports.getMovements = async (req, res) => {
    ...
    try {
        const movs = await movimiento.findMany({
            where: {
        ...
        const movements = refactorOutput(movs);

        res.status(200).json({
            status: 'OK',
            movements,
            account_saldo: calcTotal(movements),
        });
    } catch (e) {
        console.log(e);
    }
